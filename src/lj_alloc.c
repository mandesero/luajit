...
#if LUAJIT_USE_ASAN
/* conversion from malloc headers to user pointers, and back with upper redzone*/
...
static LJ_NOINLINE void *lj_alloc_malloc(void *msp, size_t nsize)
{
  ...
  size_t align_size = (size_t)align_up((void *)mem_size, SIZE_ALIGNMENT);
